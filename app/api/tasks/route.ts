...
export async function GET(req: Request) {
  ...
  const limit = 50;
  const rows = gid
    ? await sql/* sql */`select * from public.tasks where group_id=${gid}
                          order by coalesce(due_at, now()+interval '10 years') asc
                          limit ${limit}`
    : await sql/* sql */`select * from public.tasks
                          order by coalesce(due_at, now()+interval '10 years') asc
                          limit ${limit}`;
...
export async function POST(req: Request) {
  ...
  await sql/* sql */`insert into public.groups(id) values(${b.group_id})
